...
if (!empty($terms) && !is_wp_error($terms)) {
    ...
    $current_tax = is_tax('books') ? '' : 'active'; // Check if it's a taxonomy archive for any term
    echo '<a href="/books"><button class="book-category show-all ' . $current_tax . '"><span>تمام</span></button></a>';
    ...
    foreach ($terms as $term) {
        ...
        if (!is_wp_error($term_link)) {
            ...
            $active_class = (is_tax('books', $term->term_id)) ? 'active' : '';

            // Print each term as a button with a link
            echo '<a href="'. $term_link .'"><button class="book-category ' . esc_html($term->name) . ' ' . $active_class . '"><span>' . esc_html($term->name) . '</span></button></a>';
        }
    }
}
?>
