 ...
 */
  ...
  */
if ( ! function_exists( 'dawat_setup_function' ) ) {

	function dawat_setup_function() {
		// Make theme available for translation.
		load_theme_textdomain( 'dtearsd_td' );
		...
		register_nav_menus(
			array(
				'header-nav'       => __( 'Header Nav', 'dtearsd_td' ),
				'footer-nav-one'   => __( 'Footer Nav One', 'dtearsd_td' ),
				'footer-nav-two'   => __( 'Footer Nav Two', 'dtearsd_td' ),
				'footer-nav-three' => __( 'Footer Nav Three', 'dtearsd_td' ),
				'legal-nav'        => __( 'Legal Nav', 'dtearsd_td' ),
			)
		);
		...
		function nav_fallback() {
			if ( is_user_logged_in() ) {
				?> 
				<ul>
					<li> <?php _e( 'Go to admin area to create navigation menu', 'dtearsd_td' ); ?></li>
				</ul>
				<?php
			}
		}
    ...
    // Add the custom classes to the body classes array
    $classes[] = 'urdu-language';
    $classes[] = 'style-for-window';
...
add_action( 'after_setup_theme', 'dawat_setup_function' );



add_action( 'dawat_register', 'dawat_register_required_plugins' );
/**
 * Register the required plugins for this theme.
 *
 * In this example, we register two plugins - one included with the dawat library
 * and one from the .org repo.
 *
 * The variable passed to dawat_register_plugins() should be an array of plugin
 * arrays.
 *
 * This function is hooked into dawat_init, which is fired within the
 * TGM_Plugin_Activation class constructor.
 */
function dawat_register_required_plugins() {
	...
	$config = array(
		...
		'menu'         => 'dawat-install-plugins', // Menu slug.
		...
		'strings'      => array(
			'page_title'                      => __( 'Install Required Plugins', 'dtearsd_td' ),
			'menu_title'                      => __( 'Plugin Installer', 'dtearsd_td' ),
			'installing'                      => __( 'Installing Plugin: %s', 'dtearsd_td' ), // %s = plugin name.
			'oops'                            => __( 'Something went wrong with the plugin API.', 'dtearsd_td' ),
			...
			'return'                          => __( 'Return to Required Plugins Installer', 'dtearsd_td' ),
			'plugin_activated'                => __( 'Plugin activated successfully.', 'dtearsd_td' ),
			'complete'                        => __( 'All plugins installed and activated successfully. %s', 'dtearsd_td' ), // %s = dashboard link.
			'nag_type'                        => 'updated', // Determines admin notice type - can only be 'updated', 'update-nag' or 'error'.
		),
	);

	dawat( $plugins, $config );
