...
function dawat_acf_block_callback( $block ) {
  ...
  */
function dawat_blocks_category( $categories ) {
	$custom_block = array(
		'slug'  => 'dawat-blocks',
		'title' => __( 'Dawat-e-Arshad Blocks', 'dtearsd_td' ),
		'icon'  => 'dawat-blocks',
	);
...
add_filter( 'block_categories_all', 'dawat_blocks_category', 10, 2 );
...
if ( function_exists( 'acf_add_options_page' ) ) {
	$option_page = acf_add_options_page(
		array(
			'page_title' => __( 'Theme Options', 'dtearsd_td' ),
			'menu_title' => __( 'Theme Options', 'dtearsd_td' ),
 ...
 */
function dawat_acf_button( $object, $classes = '' ) {
	if ( $object['url'] ) {
		$link = '';
...
function dawat_page_title( $pagetitle ) {
	global $fields;
...
function wysiwyg_render_field_settings( $field ) {
	acf_render_field_setting(
		$field,
		array(
			'label'        => __( 'Height of Editor', 'dtearsd_td' ),
			'instructions' => __( 'Height of Editor after Init', 'dtearsd_td' ),
			'name'         => 'wysiwyg_height',
			'type'         => 'number',
		)
	);
