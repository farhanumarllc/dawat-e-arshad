 ...
 * Function used to create custom excerpt.
 */
function dawat_excerpt_nomore( $count ) {
	global $post;
	$permalink = get_permalink( $post->ID );
	$excerpt   = get_the_excerpt();
	$excerpt   = strip_tags( $excerpt );
 ...
 */
function dawat_pagination( $pages = '', $range = 4 ) {
	$showitems = ( $range * 2 ) + 1;
 ...
 */
function dawat_svg_upload_support( $mimes ) {
	$mimes['svg'] = 'image/svg+xml';
	return $mimes;
}

add_filter( 'upload_mimes', 'dawat_svg_upload_support' );


/**
 * Remove default WordPress login logo link & set it to homepage of site
 */
function dawat_login_logo_url( $url ) {
	return '"' . home_url() . '"';
}

add_filter( 'login_headerurl', 'dawat_login_logo_url' );


/**
 * Add viewport meta tag in head
 */
function dawat_viewport() {
	echo '
		<meta name="viewport" content="width=device-width, initial-scale=1.0" />
	';
}

add_action( 'wp_head', 'dawat_viewport' );
 ...
 */
function dawat_first_last_menu_classes( $items ) {
	if ( $items ) {
...
add_filter( 'wp_nav_menu_objects', 'dawat_first_last_menu_classes' );
...
function dawat_theme_archive_title( $title ) {
	if ( is_category() ) {
...
add_filter( 'get_the_archive_title', 'dawat_theme_archive_title' );
...
add_action( 'login_head', 'dawat_login_logo' );
